...
public class ArraySortDESCExam2 { // ASC = 오름차순 의미

	public static void main(String[] args) {
		// 버블 정렬 내림차순으로 구현 (IQ 150 이상)
		...
		for (int j = 0; j < scores.length - 1; j++) { // j = 0 ~ 6 까지 반복(싸이클 반복용)
			for (int k = 0; k < scores.length - 1 - j; k++) {
				if (scores[k] < scores[k + 1]) {
					int temp = scores[k]; // 작은 값이 temp 변수에 저장
					scores[k] = scores[k + 1]; // 큰 값이 k 번째에 저장
					scores[k + 1] = temp; // temp 작은 값이 k+1 번째에 저장
				} // 옆자리랑 비교하여 작은지 물어보는 if 문
			...
			System.out.println("---------배열 내림 차순 출력---------");
			for (int i = 0; i < scores.length; i++) {
				System.out.print(scores[i] + " ");
			} // for 종료
			System.out.println();
